...
from python.visualize import visualize_box_mask, visualize_attr, visualize_pose, visualize_action, visualize_vehicleplate, visualize_vehiclepress, visualize_lane, visualize_vehicle_retrograde, visualize_sudden_event
...
class PipePredictor(object):
    ...
    def is_hour(self):
        ...
        if current_time.minute == self.last_checked_minute and current_time.second == self.last_checked_second:
            return False
        self.last_checked_second = current_time.second
        self.last_checked_minute = current_time.minute
    ...
    def predict_video(self, video_file, thread_idx=0):
        ...
        out_path = os.path.join(self.output_dir,
                                "output_{}.mp4".format(datetime.datetime.now().strftime("%Y%m%d_%H-%M-%S")))
        ...
        if not writer.isOpened():
            ...
            while not writer.isOpened():
                print("Try to open VideoWriter again.")
                writer = cv2.VideoWriter(out_path, fourcc, fps, (width, height))
            print("VideoWriter reopen successfully.")
        else:
        ...
        if len(self.pushurl) > 0:
            ...
            pushurl = os.path.join(self.pushurl, video_out_name).replace("\\", "/")
        ...
        try:
            ...
            # if process_time < frame_period:
            #     time.sleep(frame_period - process_time)
            #     print("sleep time: ", frame_period - process_time)
    ...
    def visualize_video(self,
        ...
        if mot_res is not None and len(mot_res['boxes']) != 0:
            ...
            for i in classes:
                lines = mot_res['boxes'][mot_res['boxes'][:, 1] == i]
                ids = lines[:, 0]
                scores = lines[:, 2]
                boxes = lines[:, 3:]
                boxes[:, 2] = boxes[:, 2] - boxes[:, 0]
                boxes[:, 3] = boxes[:, 3] - boxes[:, 1]
                online_tlwhs[i] = boxes
                online_scores[i] = scores
                online_ids[i] = ids
        else:
            boxes = np.zeros([0, 4])
            ids = np.zeros([0])
